...
public class Picture extends SimplePicture 
{
  ...
  public void grayscale()
  {
    ...
    for (Pixel[] rowArray : pixels)
    {
      for (Pixel pixelObj : rowArray)
      {
        pixTotal += pixelObj.getRed();
        pixTotal += pixelObj.getGreen();
        pixTotal += pixelObj.getBlue();
        pixelObj.setRed(pixTotal / 3);
        pixelObj.setGreen(pixTotal / 3);
        pixelObj.setBlue(pixTotal / 3);
        pixTotal = 0;
      }
    }
}
    ...
    public void copy(Picture fromPic, int fromStartRow,int fromStartCol,int fromEndRow,int fromEndCol,int toStartRow, int toStartCol)
  {
    ...
    Pixel[][] toPixels = this.getPixels2D();
    Pixel[][] fromPixels = fromPic.getPixels2D();
    for (int fromRow = fromStartRow, toRow = toStartRow; fromRow < fromPixels.length && toRow < fromEndRow; fromRow++, toRow++)
    {
      for (int fromCol = fromStartCol, toCol = toStartCol; fromCol < fromPixels[0].length && toCol < fromEndCol;  fromCol++, toCol++)
      {
        fromPixel = fromPixels[fromRow][fromCol];
        toPixel = toPixels[toRow][toCol];
        toPixel.setColor(fromPixel.getColor());
  ...
  public void myCollage()
  {
     ...
     Picture flower2NoBlue = new Picture(flower2);
     flower2NoBlue.zeroBlue();
     this.copy(flower1, 200,200);
     this.copy(flower2, 100, 100);
     this.copy(snowman, 171,105,194,292,143, 204);
     this.mirrorVertical();
     this.copy(flower2NoBlue, 0, 0);
